...
- `NOTIFICATION_EMAIL`: The email address that should receive notifications. With Resend's test sender, this must be the email address used for the Resend account.
...
Run it again with dry-run disabled after all four secrets are configured. Existing database items do not generate an email; the next newly published item will.
